...
## まとめ
アライメントのルールは、後ろのメンバ変数のバイト数を考慮し、前からのメンバ変数の合計バイト数が8の倍数になるようにパディングされるということがわかった。なお、メンバ変数を8バイトに収めるときはできるだけ多くのメンバ変数が収まるようにされる。(例えば、構造体s4において、tag1, tag2だけでも8バイトに収まるが、idを加えても8バイトに収まるのでidも合わせて8バイトに収めるということ)