...
つまり、次に来るメンバ変数のバイト数を考慮して、8バイトの倍数になるようにアライメントされるのではないかと推察できる。次に、s3でのtagをもう一つ増やして先頭につけた以下のような構造体を考えてみる。仮説が正しければ、tag1とtag2とidの合計必要バイト数が8バイトに収まるので、tag1は1バイト、tag2はバイトパディングして3バイト、idは4バイト確保されるはずである。
```
#include <stdio.h>
