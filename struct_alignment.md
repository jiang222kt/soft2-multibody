...
## ここからわかること
s2では構造体全体の確保メモリが136バイトなので、s2のtag以降には8バイトの領域があることがわかる。これを踏まえて、s2とs3のそれぞれのメンバ変数間の領域の違いを見てみると、ageとtagでの違いに気づける。s2のtag以降には8バイトの領域があるのに対してs3のtag以降は4バイトの領域があり、また、s2のage以降は8バイトの領域があるのに対してs3のage以降は4バイトの領域がある。
  
では、それぞれの構造体について詳しく見ていこう。
  
まず、s2について、idとnameで104バイト確保していて、これはチャンク(8バイト)の倍数である。また、ageのあとのheightの型がdoubleで8バイト確保されるので、heightまでの合計領域が8バイトの倍数であることが好ましい。よって、age以降についてはint型で4バイトしか必要ないにもかかわらず、4バイトパディングして8バイト分の領域が存在する。同様に、s2のtagまでに128バイトと8の倍数分の領域があるので、tag以降では7バイトパディングして8バイトの領域が存在する。
  
次に、s3では最初にtagがあり、次にint型のidがある。この二つを合計して8バイトに収まるので、tag以降では3バイトパディングして4バイトの領域が存在する。そのあとnameで100バイト確保されて、ここまでで108バイトと8の倍数にならないが、その次のageで4バイト確保されて112バイトとなるので、この構造体ではageは4バイト確保される。

つまり、次に来るメンバ変数のバイト数を考慮して、8バイトの倍数になるようにアライメントされるのではないかと推察できる。次に、s3でのtagをもう一つ増やして先頭につけた以下のような構造体を考えてみる。仮説が正しければ、tag1とtag2とidの合計必要バイト数が8バイトに収まるので、tag1は1バイト、tag2は1バイトに2バイトのパディング、idは4バイト確保されるはずである。
```
#include <stdio.h>
...
```
これを見ると、確かにtag1は1バイト、tag2は1バイトに2バイトのパディング、idは4バイト確保されていて、仮説が正しいと考えられる。なお、なぜidのあとに2バイトパディングされるのではなくtag2のあとに2バイトパディングされるかというと、これは8バイト単位より細かい4バイト単位で考えられているためである。つまり、tag1とtag2とidでは4バイトをはみ出してしまうため、tag1とtag2と空の領域で4バイトを確保し、idで4バイトを確保するということである。そのため、tag2のあとに2バイトパディングされる。
## まとめ
アライメントのルールは、まず4バイトの倍数になるようにパディングし、そのあと8バイトの倍数になるようにパディングされるということがわかった。先程のs4の例では、tag1,tag2で2バイト確保されるので、2バイトパディングされ、そのあとidが4バイト確保される。また、なお、メンバ変数を8バイトに収めるときはできるだけ多くのメンバ変数が収まるようにされる。(例えば、構造体s4において、tag1, tag2だけでも8バイトに収まるが、idを加えても8バイトに収まるのでidも合わせて8バイトに収めるということ)